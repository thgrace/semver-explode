...
func (r *Registry) ListVersions(ctx context.Context, pkg string) ([]ecosystem.Version, error) {
	p, err := r.client.GetPackage(ctx, "npm", pkg)
	if err != nil {
		return nil, fmt.Errorf("npm: list versions for %q: %w", pkg, err)
	}
	...
	if len(out) == 0 && skipped > 0 {
		return nil, fmt.Errorf("npm: no parseable versions for %q (%d skipped)", pkg, skipped)
	}
